...
// Standalone on-robot test for the ultrasonic search state machine.
...
void setup()
{
  ...
  begin_ball_search();
  Serial.println("Ball-search test started.");
}

void loop()
{
  const BallSearchCommand command = update_ball_search();
  ...
  if (now_ms - last_status_ms >= 250)
  {
    ...
    Serial.println(command.target_found ? "yes" : "no");
  }

  // The search module keeps the motors stopped after a target is found or
  // after its bounded number of passes.
}
